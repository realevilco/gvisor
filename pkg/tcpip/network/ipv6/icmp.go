...
func (e *endpoint) handleICMP(r *stack.Route, netHeader buffer.View, pkt stack.PacketBuffer, hasFragmentHeader bool) {
	...
	case header.ICMPv6NeighborSolicit:
		...
		if !isNDPValid() || len(v) < header.ICMPv6NeighborSolicitMinimumSize {
			received.Invalid.Increment()
			return
		}
		...
		// As per RFC 4861 section 4.3, the Target Address MUST NOT be a multicast address.
	...
	case header.ICMPv6NeighborAdvert:
		...
		if !isNDPValid() || len(v) < header.ICMPv6NeighborAdvertSize {
			received.Invalid.Increment()
			return
		}
	...
	case header.ICMPv6RouterSolicit:
		...
		p := h.NDPPayload()
		if !isNDPValid() || len(p) < header.NDPRSMinimumSize {
			received.Invalid.Increment()
			return
		}
		...
		for {
			...
			switch opt := opt.(type) {
			case header.NDPSourceLinkLayerAddressOption:
				// No RFCs define what to do when an RS message has multiple Source
				// Link-Layer Address options. Since no interface can have multiple
				// link-layer addresses, we consider such messages invalid.
				if len(sourceLinkAddr) != 0 {
					received.Invalid.Increment()
	...
	case header.ICMPv6RouterAdvert:
		...
		p := h.NDPPayload()
		if !isNDPValid() || len(p) < header.NDPRAMinimumSize {
			received.Invalid.Increment()
			return
		}
