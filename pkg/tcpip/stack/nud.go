...
const (
	...
	// unit of the RetransmitTimer field in the Router Advertisement is milliseconds.
	...
	// leaves the specifics of any garbage collection mechanism up to the implementation.
...
// confirmation (e.g. ARP reply or Neighbor Advertisement for ARP or NDP, respectively).
...
type NUDConfigurations struct {
	// BaseReachableTime is the base duration for computing the random reachable time.
	//
	// Reachable time is the duration for which a neighbor is considered
	// reachable after a positive reachability confirmation is received. It is a
	...
	//
	// After this time, a neighbor entry will transition from REACHABLE to STALE state.
	//
	...
	// neighbor confirmations to anycast addresses and proxying neighbor confirmations.
...
// used instead. This is needed to check, and conditionally fix, user-specified NUDConfigurations.
...
// default specified in RFC 4861 section 10 is less than the current MinRandomFactor.
//
