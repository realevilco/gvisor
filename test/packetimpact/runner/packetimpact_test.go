...
func TestOne(t *testing.T) {
	...
	tcmd := testutil.Command(logger("cp"), "docker", "cp", "-L", *posixServerBinary, dut.Name+":"+containerPosixServerBinary)
	...
	tcmd = testutil.Command(logger("cp"), "docker", "cp", "-L", *testbenchBinary, testbench.Name+":"+containerTestbenchBinary)
	tcmd.Stdout = os.Stdout
	tcmd.Stderr = os.Stderr
	if err := tcmd.Run(); err != nil {
